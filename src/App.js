...
class App extends Component {
  ...
  clickIsVisited = id => {
    ...
    const visitCountries = countriesList.map(eachList => {
      if (eachList.id === id) {
        const updatedList = {...eachList, isVisited: !eachList.isVisited}
        return updatedList
      }
      return eachList
    })
    this.setState({countriesList: visitCountries})
  }
    ...
    return (
      <div className="app-container">
        ...
        <ul className="countries-list">
          {countriesList.map(eachCountries => (
            <CountriesList
              key={eachCountries.id}
              countryDetail={eachCountries}
              clickIsVisited={this.clickIsVisited}
            />
          ))}
        </ul>
